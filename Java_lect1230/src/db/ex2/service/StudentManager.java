...
public class StudentManager {
	...
	public boolean scoreContains(Student std, SubjectScore subjectScore) {
		...
		System.out.println(dbSubScore); 		//확인용
	...
	*/
	...
	public boolean updateScore(Student std, Subject subject, SubjectScore subjectScore) {
		...
		//새 성적의 과목의 기본키
		subjectScore.getSubject().setNum(dbSubject.getNum());
		return scoreDao.updateScore(subjectScore);	//-> 매개변수 하나로 수정 (쿼리를 단순화)
	}
	...
	public void printScore(Student std, Subject subject) {
		...
		//SubjectScore score = scoreDao.selectScoreOne(std.getKey(), subject.getNum());
		SubjectScore score = scoreDao.selectScore(tmp); //Dao는 메소드 오버로딩이 안되기 때문에 이름도 다르게 붙여야함 - >그냥 있던거 재활용
		if(score == null) {
			System.out.println("등록된 성적이 없습니다.");
			return;
		}
		System.out.println(std + " " + score);	// 객체 + 객체라 앞에 문자열 넣어줘야 문자열로 인식함
	}
}
