...
/* 외래키 update, delete 옵션
	- RESTRICT 		: 참조하고 있는 값이 있으면 삭제X, 없으면 삭제
    - CASCADE		: 참조 테이블이 기본키가 수정/삭제되면 참조하는 테이블의 외래키가 있는 데이터가 수정/삭제
    - SET NULL		: 참조 테이블의 기본키가 수정/삭제되면 참조하는 테이블의 외래키가 NULL로
    - NO ACTION		: 수정/삭제 하지 않음. RESTRICT와 동일.
